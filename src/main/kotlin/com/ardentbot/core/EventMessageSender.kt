...
class EventMessageSender {
    companion object {
        fun joinMessage(event: GuildMemberJoinEvent, register: ArdentRegister) {
            ...
            if (data.joinMessage.message != null) {
                event.user.openPrivateChannel().queue {
                    it.sendMessage("**Message from ${event.guild.name}**: ${data.joinMessage.message!!.replace("[user]", event.member!!.asMention)
                            .replace("[serversize]", event.guild.members.size.toString())
                            .replace("[server]", event.guild.name)}").queue()
                }
            ...
            try {
                event.guild.addRoleToMember(event.member!!, defaultRole).reason("Ardent default role")
                        .queue()
        ...
        fun leaveMessage(event: GuildMemberRemoveEvent, register: ArdentRegister) {
            val data = register.database.getGuildData(event.guild)
