    ...
    fun process(event: Event) {
        ...
        when (event) {
            is GuildMessageReceivedEvent -> {
                ...
                register.holder.commands.firstOrNull { it.getTranslatedName(event.guild,register) == commandName
                        ...
                        ?.let { command ->
                            ...
                            if (data.disabledModules.map { it.name }.contains(register.holder.getModuleFor(command).id)) {
                                register.sender.cmdSend(Emojis.HEAVY_MULTIPLICATION_X.cmd + "Your server administrators have " +
                                        "chosen to disable the **[]** module in Ardent".apply(register.holder.getModuleFor(command).name), command, event)
                                return
                            }
                            if (data.disabledCommands.map { it.name }.contains(command.name)) {
                                register.sender.cmdSend(Emojis.HEAVY_MULTIPLICATION_X.cmd + "Your server administrators have " +
                                        "chosen to disable the **[]** command in Ardent".apply(command.name), command, event)
                                return
                            }
                            val prefix = register.parser.lookupPrefix(event.message, prefixes)
                            val parsedMessage = register.parser.parseMessage(event.message, prefix, commandName) ?: return
